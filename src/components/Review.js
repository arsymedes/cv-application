...
class Review extends Component {
    ...
    return experiences.map((experience) => (
      <li key={experience.id}>
        ...
        <ul className="list-disc ml-10">
          {experience.points.map((element, index) => (
      ...
      <li key={education.id}>
        ...
        <div className="flex justify-between">
          <h4 className="text-md font-bold">
            {education.degree} degree in {education.fieldOfStudy}
          </h4>
          <div className="text-md font-bold">
  ...
  renderInfo() {
    ...
    return infos.points.map((info) => <li key={info.id}>{info.value}</li>);
  }
  ...
  render() {
    ...
    return (
      <div
        id="pdf"
        className="box-border self-start md:self-auto shadow-lg w-[48rem] aspect-[1/1.414] mt-8 px-14 py-12 flex flex-col items-center gap-5"
      >
        ...
        <div className="w-full flex flex-col">
          ...
          <ul className="flex flex-col gap-4">{this.renderExperience()}</ul>
        </div>
        <div className="w-full flex flex-col">
          <h2 className="text-xl font-bold mb-4 border-b-2 border-black">
            EDUCATION
          </h2>
          <ul className="flex flex-col gap-4">{this.renderEducation()}</ul>
        </div>
        <div className="w-full flex flex-col">
          <h2 className="text-xl font-bold mb-4 border-b-2 border-black">
